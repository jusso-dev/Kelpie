...
export async function GET(
  ...
  const assignees = await listAdditionalAssigneesCore(auth.token.organisationId, id);
...
export async function POST(
  ...
  const [actor] = auth.token.createdBy
    ...
    : [];
  try {
    const assignee = await addAdditionalAssigneeCore(
